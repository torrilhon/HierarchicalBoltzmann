/*************************************************************************\
 SystemB.cpp  - implements setup of model system (B)
\*************************************************************************/
#include <stdio.h>
#include <iostream>
#include <math.h>
...
// main system setup, maybe adjusted by user  
  ...
  if( SystemText.find("angular") != -1 ) {
    ...
    A1 = -0.4; 
    A2 = 0.0; 
    BC.coeffRef(0,3) = 0.0;    // this bc is not active
    BC.coeffRef(1,7) = kappa;  // kappa = 0 or 2
  };
  if( SystemText.find("radial") != -1 ) {
    ...
    A1 = 0.0; 
    A2 = -1.0; 
    BC.coeffRef(0,3) = (-kappa); // kappa = 0 or -1, negative to distinguish from case above
    BC.coeffRef(1,7) = 2.0;      // this bc is not active
  };
//---------------------------------------------------------------------------------
...
MatrixXd System::Force( MatrixXd& pos )
{
  ...
  res(0) = A0 + A2*r*r + A1*(1.0-5.0/18*r*r/(tau*tau))*pos(0)/r;
  return( res );  
};
